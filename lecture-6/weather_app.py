...
def main(page: ft.Page):
    ...
    def show_weather_page():
        try:
            # 地域情報をDBに保存
            db_manager.save_area(current_area_code, current_area_name)
            ...
            # 予報データをDBに保存
            db_manager.save_forecasts(current_area_code, forecasts_to_save)
            
            # DBから予報データを取得して表示
            db_forecasts = db_manager.get_forecasts(current_area_code, 7)
            ...
            for forecast in db_forecasts:
                ...
                _, icon_path = get_weather_info(weather_code)
    ...
    def show_city_list():
        ...
        if area_data is None:
            ...
            areas_dict = {code: info.get("name", "") for code, info in offices.items()}
            db_manager.save_all_areas(areas_dict)
